...
class FreshmanModuleReviewDetailScreen extends ConsumerStatefulWidget {
  final List<ModuleReviewRequestModel> members;
  final int currentIndex;
  final Function removeFromList;
  const FreshmanModuleReviewDetailScreen(
      {super.key,
  ...
  @override
  ConsumerState<FreshmanModuleReviewDetailScreen> createState() =>
      _FreshmanModuleReviewDetailScreenState();
}

class _FreshmanModuleReviewDetailScreenState
    extends ConsumerState<FreshmanModuleReviewDetailScreen> {
  late FToast fToast;
  ...
  void authenticate(int memberId) async {
    try {
      ...
      await ref
          ...
          .authenticateModuleReviewRequest(memberId: memberId);
      setState(() {
        isAuthLoading = false;
      });
  ...
  void reject(int memberId) async {
    try {
      ...
      await ref.read(backofficeRepositoryProvider).rejectModuleReviewRequest(
          memberId: memberId,
  ...
  void navigateToNextRequest() {
    if (widget.currentIndex == widget.members.length - 1) {
      widget.removeFromList(widget.members[widget.currentIndex].memberId);
      fToast.showToast(
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (_) => FreshmanModuleReviewDetailScreen(
  ...
  Widget build(BuildContext context) {
    ModuleReviewRequestModel currentUser = widget.members[widget.currentIndex];
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          "${currentUser.memberId}님의 회원가입 요청",
